...
//			$text->printText($img, $graphArea->x1 - 5, $y, $this->plot->getTextColor(), $value, $text->fontCondensed, $text->HORIZONTAL_RIGHT_ALIGN | $text->VERTICAL_CENTER_ALIGN); // Roberto
			$text->printText($img, $graphArea->x1 - 5, $y, $this->plot->getTextColor(), $value, $text->fontCondensedBold, $text->HORIZONTAL_RIGHT_ALIGN | $text->VERTICAL_CENTER_ALIGN); // Roberto
		}
...
//		$uppery = $graphArea->y2 - ($maxValue - $minValue) * ($graphArea->y2 - $graphArea->y1) / ($this->axis->displayDelta); // Roberto
...
//			imagerectangle($img, $x - 1, $graphArea->y2, $x, $uppery, $palette->axisColor[1]->getColor($img)); // Roberto
	...
	function printLine()
	{
		...
		$lineColorSet = new ColorSet($this->dataSet->getColorsList(), 0.75); // Roberto		
		//$lineColorSet = $palette->lineColorSet; // Roberto
		$lineColorSet->reset();
		$sizeList = $this->dataSet->getSizeList(); // Roberto
		for ($j = 0; $j < count($serieList); $j++) {
			...
			for ($i = 0; $i < $pointCount; $i++) {
				...
				// Draw line
				if ($x1) {
					$primitive->line($x1, $y1, $x2, $y2, $lineColor, $lineSize); // Roberto
					if($this->drawShadow) { // Roberto
						$primitive->line($x1, $y1 - 1, $x2, $y2 - 1, $lineColorShadow, ($lineSize / 2)); // Roberto
					} // Roberto
				}
				$x1 = $x2;
				$y1 = $y2;
...
//		$palette = $this->plot->getPalette(); // Roberto
//		$lineColorSet = $palette->lineColorSet; // Roberto
	...
	public function drawShadow($drawshadow)
	{
