...
function apri_filtro(obj, not_init)
{
	...
	url += '&FILTRO_SELEZIONA=' + escape(document.all[obj.getAttribute('FILTRO_CAMPO_VALORE')].value);
		...
		if(trim(valore) != '')
		{
			...
			else
			{
				...
				{   // se nel campo valore trovo "='" oppure "= '", aggiunge "'" in fondo 
					if (a_filtri[idx_filtri].FILTRO_CAMPO_TIPO=='TEXT' && (campo.indexOf('= \'')>0 || campo.indexOf('=\'')>0) ) {
...
NS_SALVA_FILTRO = {
    ...
    init : function(pUserLogin,pObj){
		if (typeof pObj.getAttribute('FILTRO_CAMPO_SAVE') != 'undefined'){
    ...
    save: function(){
    	...
    	try{
    		dwr.engine.setAsync(false);
    		top.dwrUtility.executeBatchStatement("gestione_wk.xml","saveFiltroWk",NS_SALVA_FILTRO.pArray,0,callBack);
    		dwr.engine.setAsync(true);
    	}catch(e){
    		try{
        		dwr.engine.setAsync(false);
    			opener.top.dwrUtility.executeBatchStatement("gestione_wk.xml","saveFiltroWk",NS_SALVA_FILTRO.pArray,0,callBack);
        		dwr.engine.setAsync(true);
    		}catch(e){
