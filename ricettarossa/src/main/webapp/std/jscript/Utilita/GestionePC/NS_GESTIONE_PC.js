...
var NS_GESTIONE_PC  = {
	...
	caricaDati:function(checkAttivaApplet){
		...
		// Carica Dati PC
		rs = opener.top.executeQuery("gestione_pc.xml","info",[document.EXTERN.IP.value]);
		if(rs.next()){
...
//				alert(stampanti.printers[i]==valueFromDb)
	...
	if (baseGlobal.SITO=='SITO'){
		$('select[name=cmbDirectoryReport]').append(NS_GESTIONE_PC.utilita.creaOption('SITO','SITO','SITO'));
	}
	else{
		$.ajax({
			url: "DirectoryReport", 
...
//		Ridimensiono i combo per la scelta delle configurazioni		
	...
	configurazioni:{
		...
		RICETTA_ROSSA : {
			...
			ORIENTAMENTO_ORIZZONTALE: '{"methods":[{"autoRotateandCenter":[false]},{"setPageSize":[8]},{"setCustomPageDimension":[210.0,297.0,4]},{"setOrientation":[1]},{"setPageScale":[1]},{"setPageMargins":[[11.0,0.0,13.0,150.0],4]}]}',
			ORIENTAMENTO_ORIZZONTALE_SAMSUNG: '{"methods":[{"autoRotateandCenter":[false]},{"setPageSize":[8]},{"setCustomPageDimension":[210.0,297.0,4]},{"setOrientation":[1]},{"setPageScale":[1]},{"setPageMargins":[[11.0,0.0,12.0,150.0],4]}]}'
