...
function mostraInfoRichiesta(posizioneArray, oggetto){
	...
	try{
		idenRichiesta = iden_worklist[posizioneArray];
		...
		sql = "select descr, descr_med_richiedente, descr_op_rich, descr_tab_spaz, quesito, quadro_cli, descr_ute_controllo, descr_ute_annulla ";
		sql += "from radsql.view_vis_richiesta where tr_iden = " + idenRichiesta;
...
function processXmlDocument(xmlDoc){
	...
	try{
		if (xmlDoc){
			...
			//DESCR_OP_RICH
			tagOperatoreRichiedente = xmlDoc.getElementsByTagName("DESCR_OP_RICH")[0];
