...
function aggiorna()
{
	...
	try{
		...
		// **** modifica 20130402
		if (doc.tipo_wk.value==""){
			doc.tipo_wk.value = "WK_AMB_RICHIESTE";
		}
		// *****
		doc.nome_vista.value = 'VIEW_AMB_RICHIESTE_WORKLIST';	
...
/*REPARTO PROVENIENZA: testata_richieste.IDEN_TAB_PRO = tab_pro.iden*/
	...
	if(filtroAggiuntivo=='true')
		{
		if (document.all.sel_aggiuntivo.value!= '')
			{
				stringa += " and filtro_aggiuntivo= '" + document.all.sel_aggiuntivo.value + "' ";
			}
			//alert(document.all.sel_aggiuntivo.value);
		}		
...
{ 
	...
	try{
		...
		alert("accettazione - Error: " + e.description);
	}
}
...
function apri_accettazione(anag_iden)
{
	...
	try{
		...
		//  "4480*@640300"
		dwr.engine.setAsync(true);
	}catch(e){alert("apri_accettazione - Error: " + e.description)}
...
function cbkTrovaTabEsaIden(tabEsaIden_anagIden)
{
	...
	try{
		...
		// ambulatorio
		url_send += "&visualizza_metodica=N&cmd_extra=hideMan();";
		if(parent.RicercaRichiesteFrame.filtroAggiuntivo=='true')
		{
			if(parent.RicercaRichiesteFrame.filtroToGenerate!='false')
			{
				if(stringa_codici(array_filtro_aggiuntivo)!='')
				{
					url_send +=eval(parent.RicercaRichiesteFrame.filtroToGenerate);
				}
			}
		}
	...
	}catch(e){alert("cbkTrovaTabEsaIden - Error: " + e.description)}
}
...
function annulla()
{
	try{
		...
		alert("annulla - Error: " + e.description);
	}
}
...
function visualizza_esami_paziente()
{
	...
	doc.action = 'worklist?namecontextmenu=WK_VIS_ESAMI_RICHIESTE';
	doc.tipowk.value = 'WK_VIS_ESAMI_RICHIESTE';
	doc.namecontextmenu.value = 'WK_VIS_ESAMI_RICHIESTE';
...
function gestioneCalendario(){
	try{
		...
		alert("gestioneCalendario - Error: " + e.description)
	}
...
function caricamento(){
	try{
		...
		alert("caricamento - error: " + e.description);
	}
}
function caricaFiltroAggiuntivo()
{
	...
	try{
		dwr.engine.setAsync(false);
		toolKitDB.getListResultData(sql, popola_combo);
		dwr.engine.setAsync(true);
	}
	catch(e){
		alert("caricaFiltroAggiuntivo - " + e.description);
	}
}



function popola_combo(elenco)
{
	try{
		for ( var c= document.all.sel_aggiuntivo.options.length -1 ; c>=0 ;c--)
		{
			document.all.sel_aggiuntivo.options.remove(c);
		}
		for (var i = 0; i<elenco.length; i++)
		{
			//
			document.all.sel_aggiuntivo.options[i]=new Option('',(elenco[i][0]),false,false);	
			document.all.sel_aggiuntivo.options[i].innerHTML = elenco[i][1];
		}
	}
	catch(e){
		alert("popola_combo - " + e.description);
	}	
