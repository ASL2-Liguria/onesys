...
{
objectNode = document.getElementById('div_Area');
objectNode.style.display='block';
	...
	if (restocdc!=0)
	{righecdc=righecdc;}
	righecdc=righecdc*25;
	restosale=nsale%3;
	righesale=(nsale-restosale)/3;
	if (restosale!=0)
	{righesale=righesale;}
	righesale=righesale*25;
	dimensione=200+righecdc+righesale;
	//alert(dimensione)
parent.document.all.oFramesetAccMultipla.rows = dimensione+",*";
...
if (open==true)
{
	...
	ncdc=document.formLL.CdcTOT.value;
	nsale=document.formLL.AreTOT.value;
	AllChiudi ('div_Area',nsale);
	AllChiudi ('div_cdc',ncdc);
	open=false;
...
else
{
	ncdc=document.formLL.CdcTOT.value;
	nsale=document.formLL.AreTOT.value;
	AllApri ('div_Area',nsale);
	AllApri ('div_cdc',ncdc);
	open=true;
}
//alert(parent.document.all.oFramesetAccMultipla.rows);
}
function aggiorna()
{}
...
function AllChiudi (valore,numerosel){
	objectNode = document.getElementById(valore);
	if (objectNode.style.display=='block'){
	objectNode.style.display='none';
	espandiFrame(numerosel,"no",0);
...
function AllApri (valore,numerosel){
	objectNode = document.getElementById(valore);
if (objectNode.style.display=='none'){
	objectNode.style.display='block';
	espandiFrame(numerosel,"si");
}
}
function espandiFrame(numeroselezionati,sino)
{
	...
	righe=righe*25;
	righe=righe+25;
	precedente=parent.document.all.oFramesetAccMultipla.rows;
	...
	if (sino == "no")
	{
		...
		parent.document.all.oFramesetAccMultipla.rows = IntPrec+",*";
	}
	else if (sino == "si")
	{
		IntPrec=IntPrec+righe;
		...
		if (MeseDa>MeseA & AnnoDa==AnnoA)
			...
			if (DayDa>DayA & MeseDa==MeseA & AnnoDa==AnnoA)
				...
				{	for(T=0 ; T<document.formLL.elements.length ; T++)
					...
					document.formAnte.DataDa.value=document.formLL.txtDaData.value;
					document.formAnte.DataA.value=document.formLL.txtAData.value;
				//	document.formNascostaStampa.stampaReparto.value=cdc;
					document.formAnte.idenSal.value=sala;
					document.formAnte.submit();
...
function creaStringaStato() {
	...
	if (document.formLL.campoStatoEnt[2].checked)
		{stringaStato="PA"+stringaStato.substring(2);}
