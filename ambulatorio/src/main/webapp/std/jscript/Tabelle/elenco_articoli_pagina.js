...
function where_condition()
{
	...
	if(doc.ricerca[0].checked)
	{
		...
		{
			where_cond += " AND descr like '%" + replace_stringa(doc.articolo.value, "'") + "%'";
		}
	...
	if(doc.ricerca[1].checked)
	{
		...
		if(doc.articolo.value != '')
		{
			where_cond += " AND cod_art like '%" + replace_stringa(doc.articolo.value, "'") + "%'";
		}
	...
	//alert(doc.hwhere_condition.value);
