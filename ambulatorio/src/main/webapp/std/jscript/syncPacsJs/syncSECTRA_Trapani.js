// JavaScript Document 
// usare Viewer3pLaunch.exe
// sintassi u user_id -s system_password -c server 
var sectraExePathDEFAULT = "\"C://SectraPolaris//Viewer3pLaunch.exe\"";
		...
		case "LOGIN":
			try{
				...
				if (webWindowTitleIDS7==""){
					...
					webWindowTitleIDS7 = "Finestra Informazioni";
				}				
			...
			//alert("handleSectra : " + handleSectra);
			if ((handleSectra !="")&&(typeof(handleSectra) !="undefined")){
				// in esecuzione
				loginToSectra();
			}
			else{
...
function launchSectraApp(){
	...
	try{
		...
		if (pathToRun==""){
			...
			// sintassi u user_id -s system_password -c server 
			// CAMBIARE !!!
			pathToRun = sectraExePathDEFAULT + " -c WS14 -u Demo -s wadm";
		}
		else{
			pathToRun = "\""+ pathToRun + "\"";
		}
...
function openStudies(patID, accessionNumber, arrayMetodica){
	...
	try{
		if (sectraObj){
			...
			//alert("call PACSShowDisplayList");
			if (bolAlmenoUno){
