...
function FILTRO_QUERY(id, txt_chk)
{
	...
	this.generateQuery = function()
	{
		if(this._sql == '')
		{
			...
			{
				this._sql += "(select distinct " + this._sql_field_value + ", " + this._sql_descr_value;
	...
	this.populateList = function(elenco)
	{
		...
		this._obj.options.outerHTML = this._obj.outerHTML.substr(0, this._obj.outerHTML.toUpperCase().indexOf('<OPTION') <= 0 ? this._obj.outerHTML.toUpperCase().indexOf('</SELECT'):this._obj.outerHTML.toUpperCase().indexOf('<OPTION')) + (this._sql_row_blank == 'S' ? '<OPTION value=""></OPTION>':'') + html + '</SELECT>';
