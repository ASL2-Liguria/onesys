...
//	initConfigurazioniFromDb();
	...
	try{
		...
		try{
			...
			// andrà sempre in eccezione perche' non si
		...
		if ((globalSito=="SAVONA")&&(abilitazioneDema=="S")){
			...
			if (document.gestione_esame.Hiden_mod_prescr.value == 103 && document.gestione_esame.tipo_registrazione.value== 'I')
				...
				// ************************
				// modifica 28-11-16
				// lo triggero se e solo se NON è dema !!!
				if (!gblScaricoDema){
				...
				if(document.gestione_esame.tipo_registrazione.value == "IA"){
					...
					/*Controllo se l'esenzione è associabile*/
					...
					/*if($("[name='Hiden_esenzione']").val()!=""){
						...
						toolKitDB.getListResultData(v_sql,function(rs){
							if(rs == 0){
								//se non è associabile allora lo sbianco e carico la fascia contrattuale se ce l'ho dall'anagrafica
	...
	// ********************** modifica per obbligatorietà campi LEA *******************
...
function continua()
{
	...
	try{
		...
		// il problema sembra presentarsi perchè se arrivo da prenotazione
...
//		if ((globalSito=="SAVONA")&&(document.gestione_esame.impegnativa.value!="")&&($("#lblImpegnativa").parent().hasClass("classTdLabelObb"))){	
		...
		// ma il flusso di lavoro è totalmente diverso ! Credo che sia una modifica fatta ad hoc per whale
...
function verifica_impegnativa(numero_impegnativa)
{
	...
	if (!gblScaricoManuale){
		if(vImpegnativa != ''){
			...
			var rs;
			// *****************
			// document.gestione_esame.Hiden_tick.value può esser vuoto !
