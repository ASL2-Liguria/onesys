function cambioCheck(chkObj)
{
	...
	{
		if((chkObj.checked && event.srcElement.nodeName == 'INPUT') || (event.srcElement.nodeName != 'INPUT' && !chkObj.checked))
		{
			chkObj.checked = true;
		}
		else
...
function applica_richieste()
{
	...
	opener.document.all.td_cdc.innerText = label_cdc;//'N° CDC sel.: ' + count;
	opener.document.getElementById('hcdc').value = cdc;
	...
	if(opener.filtroAggiuntivo=='true')
	{
			opener.caricaFiltroAggiuntivo();
	}
    self.close();
}
