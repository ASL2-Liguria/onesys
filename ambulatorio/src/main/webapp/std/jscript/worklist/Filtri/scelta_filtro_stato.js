...
function abilita_disabilita()
{
	...
	if(abil_disabil.length == 1)
	{
		...
		if (baseUser.ATTIVA_NUOVA_WORKLIST == 'S'){
			if(abil_disabil.toString().indexOf('Attivo')>0)
	...
	else
	{
		...
		{	
			if (baseUser.ATTIVA_NUOVA_WORKLIST == 'S'){
				if(abil_disabil[i].toString().indexOf('Attivo')>0)
				{
...
function applica()
{
	...
	{	
		if (baseUser.ATTIVA_NUOVA_WORKLIST == 'S'){
			if(abilitato[i].toString().indexOf('Attivo')>0)
			{
