                ...
                if(LIB.isValid(data) && LIB.isValid(data.rows))
                {
                    ...
                    $.each(data.rows, function(i, row)
                    {
                        ...
                        if(group_rows_enable)
                            objects.get('t_body').append($.createElement('tr').addClass('clsDetail').append($.createElement('td').attr('colspan', list.length)).hide());
                    });
                                ...
                                error: function(XMLHttpRequest, textStatus, errorThrown)
                                {
                                    if(XMLHttpRequest.status == 600)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            window.logger.error("wk load data [source: "+ $this.sources.get_url($this.sources.index-1)+ "] [status: " + XMLHttpRequest.status + "] [responseText: " + XMLHttpRequest.responseText + " ]");
                                            $this._root.callback.invoke("load.error", $this, {textStatus: textStatus, errorThrown: errorThrown});
                                            objects.get('loading').hide();
                                        }
                                }
