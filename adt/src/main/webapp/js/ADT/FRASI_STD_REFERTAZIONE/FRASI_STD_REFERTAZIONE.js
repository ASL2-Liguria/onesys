...
var elemento_selezionato = $("#ELEMENTO_SELEZIONATO").val();
var FRASI_STD = {
	wkFrasi:null,
	init : function() {
	...
	applica : function(pFrase) {
		var obj = top.$("#iScheda-1").contents().find('#'+elemento_selezionato);
		var testo = obj.val();
		if (testo==''){
			obj.val(pFrase);
		}
	...
	initWkFrasi : function() {
		...
		if (!FRASI_STD.divWkFrasi) {
			$("div#divWkFrasi").css({"height":"300px",width:"100%" });
            var vGruppo;
            switch ($("#PAGINA_APERTURA").val()) {
                case 'ACC_RICOVERO' :
                    vGruppo = 'DIAGNOSI_ACCETTAZIONE';
                    break;
                default :  vGruppo =  $("input#txtGruppo").val()==''?'%25':$("input#txtGruppo").val().toUpperCase();
                    break;
            }
			...
			FRASI_STD.wkFrasi = new WK({
				...
				aVal : [vGruppo, vCodice, vDescr],
				loadData:true,
                customize_params : FRASI_STD.customizeParam
			});
