...
var NS_FILTRI_STATISTICHE = {
	...
	setEvents : function(){
		...
		$("#cmbFiltro").off("change").on("change",function(){
			NS_FILTRI_STATISTICHE.handleFiltro(NS_FILTRI_STATISTICHE.getFiltroDetail($(this).find("option:selected").attr("id-filtro")));
		});
	 ...
	 * si occupa del resto. L'ordinamento è stato un ripiego in quanto il plugin non è in grado di raggruppare senza averte i dati già ordinati.
	 */
	popolaFiltri : function(){
		...
		for (var i = 0; i < NS_STATISTICA["config"]["FILTRI"]["LIST"].length; i++)
		{
			...
			if (NS_STATISTICA["config"]["FILTRI"]["LIST"][i]["GROUPABLE"]){
				opt.attr({"id-filtro":NS_STATISTICA["config"]["FILTRI"]["LIST"][i]["ID"],"key-campo":NS_STATISTICA["config"]["FILTRI"][i]["KEY_CAMPO"],"key-campo-group-order":NS_STATISTICA["config"]["FILTRI"][i]["KEY_CAMPO_GROUP_ORDER"]});
				opt.text(NS_STATISTICA["config"]["FILTRI"]["LIST"][i]["LABEL"]);
				$("#cmbGroupBy").append(opt);
			}
		}
		...
		var xhr =  db.select(
		{
			id: "ADT.Q_STATISTICHE_SALVATE",
			parameter : {
				"id_statistica":   {t: 'N', v: NS_STATISTICA["config"]["ID"]},
				"username":   {t: 'N', v: $("#USERNAME").val()}}
		});
	 ...
	 * il filtro corrispondente e lo appende a quelli già inseriti.
	 * Prima di applicare il filtro viene controllato il valore ALLOW_MULTIPLE.
	 * Se FALSE e il filtro è già stato inserito interrompe la creazione del filtro.
	...
	getFiltroSelect : function(p){
		...
		// Da un select può dipendere anche il valore di un sotto filtro
		if (p["OPTIONS"].hasOwnProperty("DATA_SLAVE")){

			select = NS_FILTRI_STATISTICHE.getSelect(p.OPTIONS);

			// Alla modifica verifico se il sottofiltro è stato valorizzato
			// in tal caso aggiorno i valori selezionati con quelli opportuni per il nuovo valore
			select.on("change",function(){
				$("#tableFiltri tr").each(function () {
					// Se il TR esaminato è tra quelli presenti nel DATA_SLAVE ([n,n1,n2...]) lo elimino e lo reinserisco
	...
	getWhereCondition : function(){
		...
		var counter = 0; // definisco un contatore esterno a causa dei multiselect che possono avere più di un valore selezionato
		...
		return {"SQL": encodeURIComponent(sql),"PARAMETERS" : whereParameters, "KEY_CONNECTION" : NS_STATISTICA["config"]["KEY_CONNECTION"], "GROUP_FIELD" : groupField.attr("key-campo"), "CAPTION" : caption};
	},
	...
	/**
	 * Una volta effettuata la selezione dei filtri è possibile salvarla.
	 ...
	 * Questo perchè tramite una chiamata POST con AJAX il client non riconosce
	 ...
	 * è possibile estrarre interamente la WK con i valori già formattati.
	 *
	 * @param nGriglia
	 */
	esporta : function(nGriglia){
		...
		var colNames = $('#colch_' + nGriglia).jqGrid('getGridParam','colNames');
		// var colData = $('#colch_' + nGriglia).getRowData(); // questa funziona ha dei problemi quando si rimuovo dalla visualizzazione delle colonne

		var colData = $('#colch_' + nGriglia).jqGrid('getGridParam','data');
		var arrayModel = $('#colch_' + nGriglia).jqGrid('getGridParam','colModel');
