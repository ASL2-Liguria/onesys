...
// JavaScript Document
		...
		{		
			...
			case 'I-PATIENT':
				if (window.user.TIPO=='I'){
					alert('Funzionalità disponibile solo per il personale medico');
				}
				else{
	...
	function creaurlbut(valore,ip,user,tipo,abilitaNewHome)
	{
		...
		{
		case 'REPERIBILITA': 
				conferma = 'Attenzione: è stata scelta la modalità operativa in regime di GUARDIA, che consente l\'accesso a dati di pazienti non ricoverati nel proprio reparto di normale competenza.\nTutte le operazioni eseguite verranno tracciate.\nContinuare?';
				break;
		default :
				conferma = 'Attenzione: è stata scelta la modalità operativa in regime di '+tipo+', che consente l\'accesso a dati di pazienti non ricoverati nel proprio reparto di normale competenza.\nTutte le operazioni eseguite verranno tracciate.\nContinuare?';
				break;
		}
