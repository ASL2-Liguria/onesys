...
function creaOpt(text, val, nameSelect){
	
	var option=opener.jQuery('<option></option>').attr("value",val.toString()).html(text.toString());
	opener.$('[name='+nameSelect+']').append(option);
...
function filtroCorpo(){
	...
	_filtro_list_corpo.setWhereBaseQuery('ORIGINE=\''+PAGINA+'\' AND CET.CODICE=COR.IDEN AND CET.TABELLA = \'MG_ART_BK_DNA_PCR\' AND CET.IDEN_TABELLA=\''+whereCond+'\'');
...
function preSalvataggio(){
	...
	document.getElementById('HelencoEsami').value=arrayAnalisi;
	document.getElementById('HelencoMateriali').value=arrayMateriali;
	document.getElementById('HelencoCorpo').value=arrayCorpo;
	document.getElementById('Hnote').value=arrayNote;
...
function selezionaEsame(){
	...
	if (combo.options.length < 1){
		...
		TABELLA.aggiungiRiga(numeroRiga, esame, hEsame,'',materiale, hMateriale);
		...
		for(var i=0;i<combo.options.length;i++){
			...
			if(combo.options[i].selected==true){
				...
				//alert('p_note'+p_note);
				TABELLA.aggiungiRiga(numeroRiga, esame, hEsame, p_note, materiale , hMateriale, combo.options[i].text, combo.options[i].value);
				controllo++;
			}
		}
...
var TABELLA = {
	...
	aggiungiRiga:function(indice, esame, iden_esame, note, materiale, iden_materiale, sede, iden_sede){
	...
	caricaAnalisi:function(){
		...
		for (var i=0;i<innerListBox.length;i++){
			...
			//TABELLA.aggiungiRiga(indice, esame, iden_esame, note, materiale, iden_materiale, sede, iden_sede)
			TABELLA.aggiungiRiga(numeroRiga, esame, iden_esame, note, materiale, iden_materiale, sede, iden_sede)
		}
	},
