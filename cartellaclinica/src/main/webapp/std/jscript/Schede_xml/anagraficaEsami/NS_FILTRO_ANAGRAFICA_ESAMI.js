...
jQuery(document).ready(function() {
    ...
    try {
    	NS_FILTRO_ANAGRAFICA_ESAMI.init();
    	NS_FILTRO_ANAGRAFICA_ESAMI.setEvents();
    } catch (e) {
        alert("NAME:\n" + e.name + "\nMESSAGE:\n" + e.message + "\nNUMBER:\n" + e.number + "\nDESCRIPTION:\n" + e.description);
    }
...
var NS_FILTRO_ANAGRAFICA_ESAMI = {
	url:"servletGenerator?KEY_LEGAME=WORKLIST&TIPO_WK=WK_ANAGRAFICA_ESAMI&ILLUMINA=illumina(this.sectionRowIndex);",	
    ...
    setEvents: function() {    	
    	...
    	$('#lblResetta').click(function(){
        	NS_FILTRO_ANAGRAFICA_ESAMI.resetta_campi();        	
        });
    	
    	$('select[name=cmbTipologiaRichiesta]').change(function() {
    		if ($('select[name=cmbTipologiaRichiesta] option:selected').text()!='RADIOLOGIA'){
        		$('select[name=cmbMetodicaRadiologica]').attr('disabled',true);    			
    			$('select[name=cmbMetodicaRadiologica]').val('');         		
    		}else{
    			$('select[name=cmbMetodicaRadiologica]').attr('disabled',false);
...
//    		NS_FILTRO_ANAGRAFICA_ESAMI.applica_filtro();
//      	}
    	$('#cmbTipologiaRichiesta').live("keypress",function(e) {    	
    		if(e.keyCode==13){
    			NS_FILTRO_ANAGRAFICA_ESAMI.applica_filtro();
    			e.preventDefault();
    		}
    ...
    applica_filtro:function(){
    	NS_FILTRO_ANAGRAFICA_ESAMI.generaWhere();
    	applica_filtro(NS_FILTRO_ANAGRAFICA_ESAMI.url);    	
    },
    resetta_campi:function(){
    	$('#txtDescrizione').val('');
    	$('#txtCodDec').val('');
		$('select[name=cmbMetodicaRadiologica]').val('');
		$('select[name=cmbTipologiaRichiesta]').val('');
    ...
    generaWhere:function(){
    	...
    	if ($('select[name=cmbTipologiaRichiesta] option:selected').val()!=''){
    		whereTmp = 'TIPOLOGIA_RICHIESTA = \''+$('select[name=cmbTipologiaRichiesta] option:selected').val()+'\''; 
			whereComponent.push(whereTmp);    		
    	}
    	
    	if ($('select[name=cmbMetodicaRadiologica] option:selected').val()!=''){
    		whereTmp = 'METODICA = \''+$('select[name=cmbMetodicaRadiologica] option:selected').val()+'\''; 
			whereComponent.push(whereTmp);    		
    	}
