jQuery(document).ready(function(){
	...
	*/
	document.getElementById('lblInterventi').ondblclick= function(){
	...
	jQuery("#lblInserisciNote").parent().mouseover(function(){
		...
		inserisciNote(document.getElementById('lblInterventi').options.value);
	});
	...
	jQuery("#lblCancella").parent().mouseover(function(){
		...
		eliminaOpt(document.getElementById("lblInterventi").options.value);
	});
});
...
function divAcr(){
	...
	creaDivACR('clsACR', 0, 'INTERVENTI_IPCCC', riempiIntervento);
...
function chiudiScheda(){
	...
	var url='servletGenerator?KEY_LEGAME=WK_IPCCC&WHERE_WK= where ID_REMOTO = '+id_remoto+' and tipo_ipccc = \'INTERVENTI\'';
...
function  riempiIntervento(iden, iden_padre, iden_figlio, descrizione ) {
	
	//funzione che riempe il campo nascosto 
	
	var codici = document.getElementById('hCodeACR').value;
	var descr = document.getElementById('hDescrACR').value;
	var iden = document.getElementById('hIdenACR').value;
	var idenScelta='';
	
	/*var splitDescr = descr.split(' / ');
	var splitCode = codici.split('/');*/
	var splitIden = iden.split('/');
	
	document.all.hIPCCC.value = iden;

	idenScelta=splitIden[splitIden.length-1];
	//alert(idenScelta + ' ' + descr);
	document.all.lblInterventi.options[document.all.lblInterventi.length] = new Option(descr, idenScelta);
	...
	jQuery("#lblInterventiIPCCC").hide().show(800);
...
function controllaLabel() {
	...
	if (document.all.hIPCCC.value == ''){
		...
		document.all.lblInterventi.innerText = '...';
...
function eliminaOpt(opt){
	...
	var myListBox = document.getElementById('lblInterventi');
...
function preSalvataggio(){
	...
	var combo= document.getElementById('lblInterventi');
	var elencoIden='';
	var elencoDescr='';
