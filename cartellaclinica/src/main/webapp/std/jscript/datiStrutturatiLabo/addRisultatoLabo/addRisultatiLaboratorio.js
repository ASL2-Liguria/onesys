...
var NS_PAGINA = {
	...
	INTESTAZIONE : {
		...
		openSceltaPrestazioni : function(){			
			...
			var finestra = window.open('servletGenerator?KEY_LEGAME=SCELTA_ESAMI_DATI_STRUTTURATI','_blank','fullscreen=yes');
		    try{
		    	parent.top.opener.top.closeWhale.pushFinestraInArray(finestra);
				}
		    catch(e){
		    	alert(e.description)
	...
	STYLES : {
		...
		setgenericStyle : function(){
			...
			if (parent._MODALITA!='RICOVERO'){
				$('#lblProvenienza').parent().parent().parent().parent().hide();
			}
	...
	ELENCO_ESAMI : {
		...
		saveRisultatiLaboratorio : function(){
			...
			if (parent._MODALITA=='RICOVERO' && $('input[name=chkProvenienza]:checked').val()==undefined) {
				alert('Attenzione, scegliere la tipologia di provenienza');
				return;
			}
			else{
				vProv=$('input[name=chkProvenienza]:checked').val()==undefined?'E':$('input[name=chkProvenienza]:checked').val();
			}
			...
			$('.trPrestazione').each(function(idx){
				...
				pBindMatrix.push([parent._V_FILTRO.idenAnag,dataOra,codEsa,vMin,vMax,vMat,result,vResp[2],vResp[3], vResp[4],vResp[5],parent._V_DATI.idRemoto , vProv, parent._V_DATI.ricovero,parent._USER_LOGIN,parent._V_DATI.reparto]);
				
				if(parent._USER_LOGIN == 'dario')
					alert('ALERT DEBUG SOLO PER ARRY \n - Iden Anag: ' + parent._V_FILTRO.idenAnag + '\n - Data Ora: ' + dataOra + '\n - Cod Esa: ' + codEsa + '\n - Val Min: ' + vMin + '\n Val Max: ' + vMax + '\n Materiale: ' + vMat + '\n Risultato: ' + result + '\n - Nome, Cognome, Data :' + vResp[2] + ' - ' + vResp[3] + ' - ' + vResp[4] + ' - ' + ' \n idpaziente: ' + parent._V_DATI.idRemoto + '  \n provenienza: ' + vProv + ' \n  nosologico:'+ parent._V_DATI.ricovero  + ' \n   login:'+ parent._USER_LOGIN + '\n - Reparto: ' + parent._V_DATI.reparto);
			});
