...
var NS_PAGINA	= {
	...
	caricamento : function(){
		if (typeof parent.parent.$('#provChiamata')!='undefined' && parent.parent.$('#provChiamata').val() =='MMG'){
...
var NS_PROFILO 	= {
	...
	caricaFiltriQuery : function(){	
		...
		_filtro_elenco_esami.setValueFieldQuery("Iden||'$'||Cod_Esa");
		_filtro_elenco_esami.setDescrFieldQuery("Descsirm");
		_filtro_elenco_esami.setFromFieldQuery("Radsql.tab_esa");
		_filtro_elenco_esami.setGroupQuery("Iden,Cod_Esa,Descsirm");
		
		// [2] Caricato Elenco Esami Profili
		_filtro_list_esa_pro = new FILTRO_QUERY("lstEsamiScelti" , null);
		_filtro_list_esa_pro.setValueFieldQuery("E.Iden||'$'||E.Cod_Esa");
		_filtro_list_esa_pro.setDescrFieldQuery("E.Descsirm");
		_filtro_list_esa_pro.setFromFieldQuery("Radsql.tab_Esa_gruppi Teg on Teg.Iden_Esa = E.Iden");
		_filtro_list_esa_pro.setGroupQuery("E.Iden,E.Cod_Esa,E.Descsirm");
	...
	caricaEsamiElenco : function(){
		...
		if((txtpresta != null) & (txtpresta != "")) { 
			whereCond += " branca='L' AND COD_ESA LIKE 'E%' and attivo='S' AND upper(Descsirm) like '%" + txtpresta + "%'";
		}
	...
	setDettaglioProfiloEsistente : function(){
		...
		listProfiliReparto = document.getElementsByName('lstProfili')[0];
		...
		{				
			...
			{					
				...
				codProfilo      = listProfiliReparto[i].value;
				descrProfilo 	= listProfiliReparto[i].text;
	...
	registraProfilo : function(){
		...
		var listEsamiScelti = document.getElementsByName('lstEsamiScelti')[0];
	...
	cancellaProfilo : function(){
		...
		var codProToDelete	= '';
		var listProfiliReparto = document.getElementsByName('lstProfili')[0];

                for (var i = 0; i<listProfiliReparto.length; i++)
		{		
			if (listProfiliReparto[i].selected)
			{			
				codProToDelete		= listProfiliReparto[i].value;
