	...
	Logger:function(pParam){
		...
		this.write = function(pText,pLevel){
			...
			if(checkLevel(this.db,this.levels[pLevel])){
				WindowCartella.executeStatement("Logger.xml","GEST_LOGS.LOG",[this.name,pText,vLevel]);
			}
...
var Popup = {
		...
		append:function(pParam){
			/*			
				[header:null]<oggetto jQuery da appendere>,
				obj:<oggetto jQuery da appendere>,
				[footer:null]<oggetto jQuery da appendere>
				[title:""]<testo da visualizzare in testata>,
				[width:320],
				[height:130],
		...
		setRemoveEvents:function(){
			...
			$("body , #AlberoConsultazione ul li").click(Popup.remove);				
			$('iframe').contents().find("body").click(Popup.remove);
		/*	$('iframe').contents().find('iframe').contents().find("body").click(Popup.remove);	*/
		/*	$('iframe').contents().find('iframe').contents().find('iframe').contents().find("body").on("click",Popup.remove);	*/
