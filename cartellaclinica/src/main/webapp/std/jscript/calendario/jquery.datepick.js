...
function Datepicker() {
	...
	this.regional = {
		'': { // US/English
			...
			prevText: '&#x3c;Prec', // Text for the previous month command
			prevStatus: 'Mese precedente', // Status text for the previous month command
			prevJumpText: '&#x3c;&#x3c;', // Text for the previous year command
			prevJumpStatus: 'Mostra l\'anno precedente', // Status text for the previous year command
			nextText: 'Succ&#x3e;', // Text for the next month command
			nextStatus: 'Mese successivo', // Status text for the next month command
			nextJumpText: '&#x3e;&#x3e;', // Text for the next year command
			nextJumpStatus: 'Mostra l\'anno successivo', // Status text for the next year command
			currentText: 'Oggi', // Text for the current month command
			currentStatus: 'Mese corrente', // Status text for the current month command
			todayText: 'Oggi', // Text for the today's month command
			todayStatus: 'Mese corrente', // Status text for the today's month command
			clearText: 'Svuota', // Text for the clear command
			clearStatus: 'Annulla', // Status text for the clear command
			closeText: 'Chiudi', // Text for the close command
			closeStatus: 'Chiudere senza modificare', // Status text for the close command
			yearStatus: 'Seleziona un altro anno', // Status text for year selection
			monthStatus: 'Seleziona un altro mese', // Status text for month selection
			weekText: 'Sm', // Text for week of the year column header
			weekStatus: 'Settimana dell\'anno', // Status text for week of the year column header
			dayStatus: '\'Seleziona\' D, M d', // Status text for selectable days
			defaultStatus: 'Scegliere una data', // Status text shown by default
...
$.extend(Datepicker.prototype, {
	...
	commands: { // Command actions that may be added to a layout by name
		...
		today: {text: 'todayText', status: 'todayStatus', // Today's month
			...
			action: function(inst) { 
				...
				$(inst.target).val($.datepick.formatDate(dateFormat, $.datepick.today(), inst.getConfig()));
				$.datepick.hide(this);
			}
		},
	...
	determineDate: function(dateSpec, defaultDate, currentDate, dateFormat, settings) {
		...
		//@francescog + @alessandroa implementato il passaggio di function come parametro		
	...
	_attachPicker: function(target, settings, opt) {			
		target = $(target);	
		if (target.hasClass(this.markerClass)) {
			return;
		}
		target.addClass(this.markerClass);
		var inst = {opt:opt, target: target, selectedDates: [], drawDate: null, pickingRange: false,
		...
		if (inst) {
			if (inst.inline) {
				...
				if($.browser.msie && parseInt($.browser.version, 10) < 7){
					var ret =  window.showModalDialog("cal.html",inst.opt,"dialogWidth:258px;dialogHeight:255px;scroll:no;status:no;help:no");
                    $.datepick.hide($.datepick.curInst);
					if(typeof ret != 'undefined' && ret != null && ret != ""){
						$(inst.target).val(ret);$(inst.target).focus();
					}
					$.datepick.hide($.datepick.curInst);
				}else if($.browser.chrome) {
					target.html(this._generateContent(target[0], inst));
				}else{
					var ret =  window.showModalDialog("cal.html",inst.opt,"dialogWidth: 152px; dialogHeight: 225px; scroll:no ; status:no ");
                    $.datepick.hide($.datepick.curInst);
				}
				if(typeof ret != 'undefined' && ret != null && ret != ""){
					$(inst.target).val(ret);$(inst.target).focus();
	...
	_generateContentWithoutAppend: function(target, inst) {
		var renderer = inst.get('renderer');
		var monthsToShow = inst.get('monthsToShow');
		monthsToShow = ($.isArray(monthsToShow) ? monthsToShow : [1, monthsToShow]);
		inst.drawDate = this._checkMinMax(
...
$.fn.datepick = function(options) {
	...
	return this.each(function() {		
		if (typeof options == 'string') {
			...
			$.datepick._attachPicker(this, options || {}, options);
		}
	});
};
