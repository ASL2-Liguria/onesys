...
var NS_DIARI_DIET = {
    ...
    // imposto data inizio ricovero e iden_visita per la visualizzazione dei diari diet
    switch (WindowCartella.FiltroCartella.getLivelloValue(vDati)) {
        case "IDEN_ANAG": 
        case "ANAG_REPARTO":
            data_ini_rico = WindowCartella.getPaziente("DATA"); 
            $("#hIden").val('');
            break;
        case "NUM_NOSOLOGICO":
            data_ini_rico = WindowCartella.getRicovero("DATA_INIZIO");
            $("#hIden").val('');
            break;
        case "IDEN_VISITA":
            data_ini_rico = WindowCartella.getAccesso("DATA_INIZIO");
            $("#hIden").val(vDati.iden_visita);
            break;
	};
    ...
    $('form[name="EXTERN"] input[name="WHERE_WK_EXTRA"]').val("(IDEN_VISITA="+vDati.iden_ricovero+" or PARENT="+vDati.iden_ricovero+") and TIPO_DIARIO='DIETISTA'");     
...
function aux_applica_filtro() {
    ...
    var url = "servletGenerator?KEY_LEGAME=WORKLIST&TIPO_WK=WK_DIARI_DIET"+contextMenu+"&ILLUMINA=javascript:illumina(this.sectionRowIndex);";
    applica_filtro(url);
}