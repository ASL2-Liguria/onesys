...
{	
		...
		if($('#txtDataNasc').val() != ""){
			...
			if(dataN > 0){
				...
				$('#txtEtaMesi').val(getAgeMonths($('#txtDataNasc').val()));
					...
					if (getAgeMonths($('#txtDataNasc').val())==0){
						...
						$('#lblCodFisc').removeClass("lblCodFisc");
						$('#lblCodFisc').unbind('mouseenter mouseleave');                
						$('#lblCodFisc').unbind('click');
	...
	if((baseGlobal.SITO=='ASL2' && $("#txtID4").val() != "") || (baseGlobal.SITO!='ASL2' && $("#txtID2").val() != ""))
	{
		// Il comune di residenza deve poter essere comunque modificato
		$("input[STATO_CAMPO='O'][name!=txtLocalita]").attr("readonly","readonly").addClass("inputReadOnly");
	...
	{
	     var statoCampoCodiceFiscale = $('#txtCodFisc').attr("STATO_CAMPO");
	 		//	Calcolo Codice Fiscale
	            if(statoCampoCodiceFiscale === 'I' || statoCampoCodiceFiscale==='E' || getAgeMonths($('#txtDataNasc').val())==0 ){
	                $('#lblCodFisc').addClass("lblCodFisc");
	                $('#lblCodFisc').hover(function(){$(this).css("cursor","pointer");},function(){$(this).css("cursor","default");});                
	                $('#lblCodFisc').click(function()
	                {
	...
	$('#lblCivico,#txtCivico,#lblCivico2,#txtCivico2').hide();
	...
	dwrUtility.executeQuery("anagrafica.xml","getEsenzioni",[$('#txtID1').val()],function(resp){
		if(resp[0][0]=='KO'){
			alert(resp[0][1]);
		}else{
			for (var i =2; i < resp.length; i++) {
			    $('SELECT[name=esenzioniAnagrafe]').append($('<option>', {
			        text:resp[i][0]
			    }));
			}
		}
...
/*	if(($('input[name="hCodCom"]').val().substr(0,1)) != 'Z'){
	...
	if(!verificaCodiceFiscaleDefinitivo($('#txtCodFisc').val()) && !verificaCodiceFiscaleProvvisorio($('#txtCodFisc').val()) ){
		return alert("Attenzione, codice fiscale inserito non valido");
	}
	...
	pBinds.push($('#txtCodFisc').val().toUpperCase());	
	if($('#IDEN_ANAG').val()==null){pBinds.push(0);}else {pBinds.push($('#IDEN_ANAG').val());
		}
	
	dwr.engine.setAsync(false);
	dwrUtility.executeQuery("anagrafica.xml","getAnag",pBinds,function(resp){
		if(resp[0][0]=='KO'){
			alert(resp[0][1]);
		}else{
			if (resp[2][0] =='1'){
			alert("Il codice fiscale inserito è associato ad un'altra anagrafica, impossibile proseguire");
			return;
			}
			else{
	             controllaDatiFondamentali();
			}
		}
...
function controllaDatiFondamentali(){
	
	var pBinds = new Array();	
	pBinds.push($('#txtNome').val().toUpperCase());
	pBinds.push($('#txtCogn').val().toUpperCase());
	pBinds.push($('#txtDataNasc').val().toUpperCase());
	pBinds.push($('input[name="radSesso"]:checked').val());
	pBinds.push($('input[name="hLuogoNasc"]').val());	

	if($('#IDEN_ANAG').val()==null){pBinds.push(0);}else {pBinds.push($('#IDEN_ANAG').val());}
	
	dwr.engine.setAsync(false);
	dwrUtility.executeQuery("anagrafica.xml","getAnagDatiFondamentali",pBinds,function(resp){
		if(resp[0][0]=='KO'){
			alert(resp[0][1]);
		}else{
			if (resp[2][0] =='1'){
			alert("Esiste un paziente con gli stessi dati fondamentali, ma un codice fiscale differente da quello inserito, impossibile proseguire");
			return;
			}
			else{
				// modifica aldo 12-14
...
function chiudi_anag(allinea)
{
	...
	if(_INSERISCI_ESAME == 'S'){
		...
		try{
			...
			var provenienza = opener.parent.idRicPazRicercaFrame.document.form_pag_ric.provenienza.value;
			...
			// graziav bug 3593
			// solo per ASL2, in inserimento paziente, chiamo la funzione di allineamento esenzioni
			if (typeof document.EXTERN.IDEN_ANAG == 'undefined' && baseGlobal.SITO=='ASL2' && allinea=='S'){
				
				var pBinds = new Array();	
				pBinds.push($('#txtNome').val().toUpperCase().replace("'","''"));
				pBinds.push($('#txtCogn').val().toUpperCase().replace("'","''"));
				pBinds.push($('input[name="radSesso"]:checked').val());
				pBinds.push($('#txtCodFisc').val().toUpperCase());	
			
				dwr.engine.setAsync(false);
				dwrUtility.executeStatement("anagrafica.xml","allineaEsenzioni",pBinds,0,function(resp){
				null;
				});
				dwr.engine.setAsync(true);					
			}
...
/* modifica Aldo 12-14 */
function sbloccaReadOnly(){
	var bolEsito = false;
	dwr.engine.setAsync(false);
	dwrUtility.executeStatement("anagrafica.xml","sbloccaReadOnly",[jQuery('#IDEN_ANAG').val()],0,function(resp){
		if (resp[0]=="OK"){
			bolEsito = true;
		}
		else{
			alert("Errore nello sblocco angrafica - " + resp[1]);
		}
	});
	dwr.engine.setAsync(true);	
	return bolEsito;
...
function checkText(cf){var re=/^([0-9a-zA-Z\xE0\xE8\xE9\xF9\xF2\xEC\x27\x40\x2D]\s?)+$/;var verifica=cf.match(re);if(!verifica&&cf!=''){return false;}return true;}
