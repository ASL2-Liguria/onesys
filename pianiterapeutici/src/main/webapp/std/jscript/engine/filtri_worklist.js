	...
	if(typeof a_obj_value != 'undefined' && typeof a_obj_descr != 'undefined')
	{
		...
		if(trim(ret_valore) != '')
		{
			...
			//self.close();
                ...
                {
                    alert("Selezionare almeno un dato!");
                }
	}
}
...
function chiudi()
{
    if(typeof document.all['FILTRO_CALL_AFTER_CLOSE'] != 'undefined')
        if(document.all['FILTRO_CALL_AFTER_CLOSE'].value != '')
            eval('opener.' + document.all['FILTRO_CALL_AFTER_CLOSE'].value);
