...
<head>
    ...
    <link rel="stylesheet" href="../../css/LOGIN_PAGE/login.css" type="text/css"/>
    <link rel="stylesheet" href="../../css/wk.css?t=7" type="text/css"/>
    <link rel="stylesheet" href="../../css/zebra_datepicker.css?t=73" type="text/css"/>
    <script type="text/javascript" src="../../js/Base/LIB.js?t=74"></script>
    <script type="text/javascript" src="../../js/Base/NO-min/NS_DATE.js?t=73"></script>
    <script type="text/javascript" src="../../js/Base/NO-min/jquery.dialog.js?t=74"></script>
    ...
    <script type="text/javascript" src="../../js/Base/NS_WORKLIST_COMPONENT.js?t=73"></script>
    <script type="text/javascript" src="../../js/Base/NO-min/jquery.contextmenu.js?t=73"></script>
    <script type="text/javascript" src="../../js/Base/worklist.js?t=73"></script>
    <script type="text/javascript" src="../../js/Base/NS_PROCESS_CLASS.js?t=73"></script>
    <script type="text/javascript" src="../../js/Base/NO-min/WK.js?t=73"></script>
    <script type="text/javascript" src="../../js/Base/NO-min/jquery.zebra_datepicker.js?t=73"></script>
    <link rel="stylesheet" href="../../css/eoc.css" type="text/css"/>
...
<div class id="divFormLogin"  style="width: 600px" >
    ...
    <div class id="contentLogin">
        <div style="display: block;">
            ...
            <%
                ...
                catch (Exception e)
                {
                    try {
                        ...
                        ApplicationContext  context_spring = new FileSystemXmlApplicationContext(ElcoContextInfo.getContextPath() +  ElcoContextInfo.getSystemParameter("contextConfigLocationLDAP"));
                        ElcoLDAP ldapSearch = context_spring.getBean("ldapSearchAd", ElcoLDAP.class);

                        result=ldapSearch.ExecuteUserSearchFromAD(name);
                        iCallProcedureFunction callFunction = CaronteFactory.getFactory().createCallFunction("POLARIS_DATI","GECO");
                        callFunction.setSqlInParameter("parametriAdChiavi", result.keySet().toArray(), OracleTypes.ARRAY);
                        callFunction.setSqlInParameter("parametriAdValori", result.values().toArray(), OracleTypes.ARRAY);
                        callFunction.setSqlInParameter("user", name, OracleTypes.VARCHAR);
                        name = (String)callFunction.execute("EOC_GESTIONE_UTENTI_AD");
                        if(name.indexOf("KO")<0)
                        {
                            baseFactory bf2=new baseFactory();
                            iBaseUser ib =   bf2.createBaseUser(name,"RIS","1");
                            baseFactory.setBaseUser(session,ib);
                        }
                    } catch (Exception e1) {
                        ...
                        {
                            name="KOjQueryError " + name + " result:" + " Errore:"+ e1.getMessage();
                        }
            ...
            <input type="hidden" id="fid" value="<% out.print(request.getParameter("FID")); %>">
        ...
        <script type="text/javascript">
            ...
            function CreaWorklistPaziente()
            {
                ...
                if(utente == null || utente =='null' || utente =='' || utente.indexOf('KOjQuery')>=0)
                {
                    document.getElementById('MESSAGE_ERROR').value='UTENTE NON IDENTIFICATO CORRETTAMENTE \r\nTENTATIVO DI IMPORTAZIONE DA SISTEMA OSPEDALIERO \r\nATTENDERE PREGO...'	+ '\r\n' + utente;
                    return;
                ...
                if(pid != null)
                {
                    ...
                    toolKitDB.getResult("EOC.GET_IDEN_ANAGRAFICA_FROM_PID",param,null,function(resp){
                ...
                {
                    document.getElementById('MESSAGE_ERROR').value='NE PID NE NUMERO CASO PERVENUTO'	;
                    return;
            ...
            function InserisciRichiesta()
            {
                ...
                var url='&CALL_ON_CLOSE=AggiornaWorklist&scheda=SCHEDA_ESAME::CLOSE_ALL=S::STATO_ESAME=2::RICHIESTA=S::IDEN_ANAGRAFICA='+jQuery("#iden_anagrafica").val();
                var urlFinal= jQuery().getAbsolutePathServer() + 'Autologin?NO_APPLET=S&username=' +jQuery("#utente").val() + '&IDEN_ANAGRAFICA=' +jQuery("#iden_anagrafica").val() + url ;
                //alert( window.location) ;
                var fnd=window.open(urlFinal,"","width="+  screen.width + ",height=" +  (screen.height-100) +",top=0px,left=0px,fullscreen=true,statusbar=0")

            }
            function ApriRichiesta(id_testata,stato_testata)
            {

              if(stato_testata == 'I')
                {
                    var url='&CALL_ON_CLOSE=AggiornaWorklist&scheda=SCHEDA_ESAME::CLOSE_ALL=S::STATO_ESAME=2::RICHIESTA=S::IDEN_ANAGRAFICA='+jQuery("#iden_anagrafica").val()+'::IDEN_TESTATA=' + id_testata;
                    var urlFinal= jQuery().getAbsolutePathServer() + 'Autologin?NO_APPLET=S&username=' +jQuery("#utente").val() + '&IDEN_ANAGRAFICA=' +jQuery("#iden_anagrafica").val() + url ;
                    //alert( window.location) ;
                    var fnd=window.open(urlFinal,"","width="+  screen.width + ",height=" +  (screen.height-100) +",top=0px,left=0px,fullscreen=true,statusbar=0")
                }
               else
                  {
                      ...
                      var msg = "Impossibile modificare, richiesta gia presa in carico dalla radiologia";
                      jQuery.dialog(msg,{
                          title:"Attenzione",
                          width:350,
                          showBtnClose:true,
                          movable:false,
                          createOn:jQuery("body"),
                          buttons:[
                              {label:"OK",action:function(ctx){ctx.data.close();}}
                          ]
                      });
                  }
            }
                ...
                jQuery.ajax(
                        {
                            type: 'POST',
                            dataType: 'text',
                            url: jQuery().getAbsolutePathServer() + 'pageWorklistStructure?ID_WK='+params.id,
        ...
        <SCRIPT>
            var NS_DEFAULT={
                da_data : moment().add('days', - 3650).format('YYYYMMDD'),
                a_data  : DATE.getOggiYMD(),
                da_data_visuale:  moment().add('days', - 3650).format('DD/MM/YYYY'),
                a_data_visuale:   moment().format('DD/MM/YYYY')
            };
            var Fileserver="http://172.30.119.14/Download/getReferti.php?ID=" ;
...
<div  id="divOk" style="width: 1000px;height: 830px;display: none;float: left">
    ...
    <table id="tabFiltri">
     <tr>
         ...
         <td>
             <button id="butNuovaRichiesta"onclick="javascript:InserisciRichiesta()">Inserisci Nuova Richiesta</button>
         </td>
         <td>
