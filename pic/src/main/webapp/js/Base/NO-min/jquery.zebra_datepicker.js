...
;(function($) {
    ...
    $.Zebra_DatePicker = function(element, options) {
        ...
        var defaults = {
            ...
            onSelect:           function(){$element.focusout(); },
            ...
            // se settato a true inizializza il campo con la data odierna
        ...
        var init = function(update) {
            ...
            if (plugin.settings.readonly_element) $element.attr('readonly', 'readonly');     //AGGIUNGERE IN OPZIONE
            ...
            //alert("el: "+$element.val());
             var hEl=  $("#h-"+$element.attr("id"));
           // alert(hEl.val());
            if(hEl.val() == "" && plugin.settings.startWithToday){
            	select_date ( current_system_year, current_system_month, current_system_day, "none", $(this));
            }
            ...
            if (!plugin.settings.always_visible) {
               ...
               // alert(element_width+" "+element_margin_left);
                ...
                /*    icon.css({
                	...
                	/* icon.css({
                         'margin-left':  element_width - icon_width - 10,    //IE
                         'margin-top': '-20px' /* (-(element_height - icon_height) / 2)
                     }); */
                
              // alert('left:' + (element_position.left + element_margin_left + element_width - icon_width)+'top:'+
            //		   ( element_position.top + element_margin_top + ((element_height - icon_height) / 2)));
            ...
            if (undefined != icon)
               ...
               // if (!($element.is(':visible'))) icon.css('display', 'none'); else icon.css('display', 'block');   Jacopo: tolto perkè non si vedeva l'icona nel filtri dei tab nscosti
            ...
            daypicker.delegate('td:not(.dp_disabled, .dp_weekend_disabled, .dp_not_in_month, .dp_week_number)', 'click', function() {
                ...
                // se show_week_number è specificato, seleziono il numero della settimana relativa al giorno cliccato
                if( plugin.settings.show_week_number !== false ) {

                    var
                        default_date = new Date(selected_year, selected_month, to_int( $(this).html() ) ),
                        selected_value = format(default_date);

                    $element.val(selected_value);

                    $( "#h-"+ $element.attr("id")).val( $(this).parent().find("td.dp_week_number").text() );
            ...
            /**
             * Formatta il valore ricevuto in hidden
             */            
            var idEl = $element.attr("id");
            if($("#h-"+idEl).val() != "")
        	{
            	plugin.setDataIso($("#h-"+idEl).val());
        	}

        }
        
        /**
         *	Imposta la data a partire da una data in formato YYYYMMDD
         */
        plugin.setDataIso = function(dataIso) 
        {
        	//alert("ISO: "+dataIso);
        	$element.val(DATE.format({date:dataIso,format_out:"DD/MM/YYYY"}));
        	var idEl = $element.attr("id");
        	$("#h-"+idEl).val(dataIso);
        ...
        var select_date = function(year, month, day, view, cell) {
                  ...
                  // alert("select_date: "+year+"-"+month+"-"+day+"-"+view+"-"+cell);
            var
            ...
            // updates value for the date picker whose starting date depends on the selected date (if any)
            update_dependent(default_date);
            plugin.setDataIso(year +  str_pad(month + 1, 2) +  str_pad(day, 2)) ;
            // if a callback function exists for when selecting a date
            if (plugin.settings.onSelect && typeof plugin.settings.onSelect == 'function')
                // execute the callback function
                plugin.settings.onSelect(selected_value, year +  str_pad(month + 1, 2) +  str_pad(day, 2), new Date(year, month, day)); // modificato parametro 2 con YYYYMMDD
...
var FIELD_DATA={
  crea:function(param){
      var $data = $("<input>",{"type":"text","id":param.Id,"name":param.Id}).css({'width':120});
      $data.after($("<input>",{"type":"hidden","id":"h-"+param.Id,"name":"h-"+param.Id}));
