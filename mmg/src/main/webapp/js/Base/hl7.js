/**
 * Created by fabioc on 13/08/2015.
 */
var HL7=
{
    dictionary :
    ...
    _parsializza: function(msg)
    {
        var segments = [];
        var  fields = [];
        var messaggio = {}
        segments = msg.split('<br/>') ;
            ...
            if(v != null &&v != '' )
            {
                ...
                $.each(fields,function(k1,v1){
                    if(k1>0) {
                        var descrCampo =  ''
                        try{
                            descrCampo =   HL7.dictionary[fields[0]][k1]
                        }
                         catch(e)
                         {}
                        campo.push({"idx": k1 , "descr":descrCampo, "value": v1});
                    }
                });
                //alert(lamponi);

                messaggio[fields[0]] = campo;
            //alert(banane);
            }
    ...
    getArraySegment: function(msg,segment)
    {
        return HL7._parsializza(msg)[segment];
    },
    getArraySegments: function(msg)
    {
         return HL7._parsializza(msg);
    },
    getCampo: function(msg,segment,posizione)
    {
        return HL7._parsializza(msg)[segment][posizione].value;
    },
    getSegmentsString: function(msg)
    {
        var  seg = [];
        segments = msg.split('<br/>') ;
        $.each(segments,function (k,v) {
            if(v != null &&v != '' ) {
                seg.push({"chiave":v.split('|')[0],"valore":v})
            }
        })
        return  seg;
    }
