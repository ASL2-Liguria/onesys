...
var NS_LOGIN = {
    ...
    init: function()
    {
        ...
        try
        {
            nomeHost = AppStampa.GetCanonicalHostname(); /*.toUpperCase()*/
        }
        catch (ex)
        {
            if (window.console) {
                console.log("Java error [\"var nomeHost = AppStampa.GetCanonicalHostname();\"]"); /*.toUpperCase()*/
			}
			nomeHost = "";
        }

        this.setHostname(nomeHost);
            ...
            success: function (response)
            {
                ...
                switch (resp)
                {
                    ...
                    case 'FF':
                        ...
                        /*se faccio la login con smartcard*/
                        if($("#codice").val() != "")
                        {
                            $("#username").val("");
                            //$("#forceLogout").val("S");
                        ...
                        var dial = $.dialog("Disconnettere utente gia autenticato su <b>" + obj.IP + "</b> ultimo logout: <b>" + obj.DATA_ACCESSO + "</b>?",
                            {
                                id: "dialogGiaAutenticato",
                                title: traduzione.alreadyAuthTitolo,
                                width: 350,
                                showBtnClose: false,
                                movable: true,
                                buttons: [
                                    {
                                        label: traduzione.alreadyAuthContinua, action: function (ctx)
                                        {
                                            $("#forceLogout").val("S");
    ...
    getVersion:function(){
        $.getJSON("version.json",function(v){
            if(v)
                $("#h5VersionVal").html(v['version']);
        });
    },
    setHostname:function(nomeHost){
...
var NS_CAMBIA_LOGIN = {
    ...
    dwrCambiaPassword: function(ctx, data)
    {
        ...
        var dbParams={
            p_username:{v:data[0].value,t:'V'},
            p_old_password:{v:data[1].value,t:'V'},
            p_new_password:{v:data[2].value,t:'V'}
        }
        var db = $.NS_DB.getTool({"setup_ajax":{"headers":{"CHECK_SESSION":"N"}}});
        var xhr = db.call_function(
            {
                datasource: 'CONFIG_WEB',
                id: 'UTILITY_WEB.CAMBIA_PASSWORD',
                parameter: dbParams
            });
        ...
        xhr.done(function (response) {
            ...
            if(response.p_result.split("$")[0]=="OK")
                    {
                NS_CAMBIA_LOGIN.okCambiaPassword(data[2].value, ctx);
                    }
            else{
                NS_CAMBIA_LOGIN.koCambiaPassword(response.p_result.split("$")[1]);
                }
            });

        xhr.fail(function (jqXHR, textStatus, errorThrown){
            NS_LOADING.hideLoading();
            NS_CAMBIA_LOGIN.koCambiaPassword(textStatus);
        });
